...
本产品是单用户、本地优先的工具，无需登录或注册。所有任务、睡眠记录和设置数据只存在于设备的沙盒中，用户可以随时导出或重置数据，但换机/跨设备同步不在当前版本范围。
...
- **数据管理（Data Management）**：所有数据仅保存在 SwiftData，用户可导出 CSV/JSON 备份或清除，暂不提供云同步或可穿戴集成。
...
**数据存储：** 打卡数据完全保存在本地（SwiftData），App 不依赖网络即可完成整个记录与分析流程。
...
**本地数据库：** SwiftData。作为 iOS 17+ 原生推出的现代数据持久化框架，SwiftData 与 SwiftUI 深度集成。所有睡眠记录、任务、提醒和用户配置均写入本地模型容器，应用可以离线完成整个 CBTI 流程。

**本地优先策略：** 数据默认保存在设备沙盒，只有在用户主动操作时（如导出 CSV/JSON）才可能移动数据。未来的换机/备份流程可以通过手动导出+导入来支持，但不依赖网络。

**数据加密：** SwiftData 使用系统级文件加密；必要时可叠加 SQLCipher 进行应用级加密。关键配置（如提醒时间）也存放在密钥受保护的 UserDefaults。

**网络依赖：** 当前版本不依赖任何远程服务，完全在设备端运行，不需要登录、认证或云同步。

### 4.4 第三方SDK与API

**推送通知：** 使用 UserNotifications 计划本地提醒（睡前/醒后/复诊提醒），避免引入远程推送服务。

**动画与媒体：** 可选使用 Lottie 载入设计师提供的引导动画，其余交互动效由 SwiftUI 原生动画完成。

**数据分析：** 所有统计与趋势分析在本地计算，依赖 Swift 语义工具（Swift Algorithms）与 Swift Charts，不调用外部分析服务。
...
│  ┌────────────┐  ┌──────────────┐  ┌───────┐ │
│  │ 视图层     │  │ 领域/业务逻辑 │  │ 数据层 │ │
│  │ (SwiftUI)  │  │ (Use Cases)  │  │ (SwiftData) │ │
│  └────────────┘  └──────────────┘  └───────┘ │
└─────────────────────────────────────────────┘
```

所有模块都运行在设备端：SwiftUI 视图通过 ViewModel 触发领域逻辑，再访问 SwiftData 数据层。数据写入后可立即恢复，整个流程不依赖网络即可完成 CBTI 管理。
...
**Data层：** 负责数据获取（本地 SwiftData、系统服务）和数据映射。实现 Domain 层定义的 Repository 接口。
...
| 层级 | 技术选型 | 说明 |
|------|----------|------|
| iOS 前端 | SwiftUI | 声明式 UI+Flow，适配 iOS 17+ |
| 本地存储 | SwiftData | 所有记录/设置直接写入本地容器 |
| 图表 | Swift Charts | iOS 原生图表库，展现趋势与关联分析 |
| 通知 | UserNotifications | 本地提醒（睡前、醒后、复诊） |
| 动画（可选） | Lottie | 用于轻量引导动画，非核心 |
...
**本地备份：** 定期维护最近30天数据快照，保存在 SwiftData 容器内。用户可在设置界面手动创建本地副本并保存在「文件」应用中。

**导出功能：** 支持导出数据为 CSV 或 JSON，便于用户与医生分享或通过「文件」迁移到其他设备。

**隐私控制：** 明确告知用户数据仅保存在设备，提供清空数据和重置引导的入口，避免误删造成困扰。
...
| reminder_preferences | JSON | 睡前/醒后提醒设置 |
...
此阶段确保产品质量达到发布标准。执行全面测试，包括功能测试、性能测试、兼容性测试（iOS 15+）。进行隐私合规审查，确保符合 App Store 健康类 App 审核要求。优化交互动效与可用性细节，准备应用商店素材（截图、描述、隐私策略）。提交审核并规划首批用户反馈机制。
